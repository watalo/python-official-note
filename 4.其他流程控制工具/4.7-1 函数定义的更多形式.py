...
parrot(1000)                                          # 1 位置参数
parrot(voltage=1000)                                  # 1 使用一个关键字参数
parrot(voltage=1000000, action='VOOOOOM')             # 2 个关键字参数
parrot(action='VOOOOOM', voltage=1000000)             # 2 两个关键字参数可以不考虑位置
parrot('a million', 'bereft of life', 'jump')         # 3 不使用关键字的时候要注意位置的顺序
parrot('a thousand', state='pushing up the daisies')  # 1 只要是有位置参数，就要考虑位置参数的顺序
